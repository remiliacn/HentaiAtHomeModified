...
public class CacheHandler {
    ...
    public CacheHandler(HentaiAtHomeClient client) throws java.io.IOException {
        ...
        File[] files = Settings.getTempDir().listFiles();
        if (files == null) {
            HentaiAtHomeClient.dieWithError("Insufficient I/O permission.");
            return;
        }
        ...
        if (!fastStartup) {
            ...
            staticRangeOldest = new Hashtable<>((int) (Settings.getStaticRangeCount() * 1.5));
    ...
    private String writeCacheObject(File file, Object object) throws IOException {
    ...
    public boolean recheckFreeDiskSpace() {
        ...
        if (bytesToFree > 0 && cacheCount > 0 && Settings.getStaticRangeCount() > 0) {
            ...
            while (staticRanges.hasMoreElements()) {
                ...
                long thisRangeOldestAge = staticRangeOldest.get(checkStaticRange);
    ...
    private long getLruLastModifiedPruneCutoff(long oldestRangeAge, long nowtime) {
        ...
        return lruLastModifiedPruneCutoff;
    }
