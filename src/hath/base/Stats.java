...
public class Stats {
    ...
    private static final List<StatListener> STAT_LISTENERS = new ArrayList<>();
    ...
    public static void addStatListener(StatListener listener) {
        synchronized (STAT_LISTENERS) {
            if (!STAT_LISTENERS.contains(listener)) {
                STAT_LISTENERS.add(listener);
            }
        }
    }
    ...
    private static void statChanged(String stat) {
        ...
        if (announce) {
            synchronized (STAT_LISTENERS) {
                for (StatListener listener : STAT_LISTENERS) {
